...
function generateRequestAndResponse(body, options, res, datas) {
	...
	if (requestBody) ret.req = JSON.stringify(requestBody);
	if (responseBody) ret.res = JSON.stringify(responseBody);
	// if (res.headers) {
	// 	ret.res.headers = inspect(res.headers);
	// }
		...
		module.request = function(options, callback) {
			...
			req = oldRequest.call(http, options, function(res) {
				...
				res.once('end', function() {
					...
					outputs.push(out);
					if (! dont_print) { console.log(cutBegin + out + cutEnd); }
				});
