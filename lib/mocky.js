...
module.exports.createServer = function(routes, params) {
	...
	var params = params || {},
		...
		reqSeq = 0;
	//transforms req/res routes to objects
	routes.forEach(function(route) {
		if ('req' in route) route.req = normalizeReqRes(route.req);
		if ('res' in route) route.res = normalizeReqRes(route.res);
	});
	function normalizeReqRes(reqres) {
		if (isString(reqres)) reqres = {body: reqres};
		return reqres;
	}
	return require(srvType).createServer(function(req, res) {
			...
			if (matched.length) {
				if (matched.length == 1) {
					...
					if (isFunction(routerRes)) {
						routerRes = normalizeReqRes(routerRes(req, reqBody));
					}
		...
		function reqMatcher(req, reqBody) {
			return function(route) {
				var isRegexRoute = typeof route.url === 'object' &&
						typeof route.url.test === 'function';
				if (req.method == route.method &&
						(isRegexRoute ?
						route.url.test(req.url) : req.url === route.url)) {
